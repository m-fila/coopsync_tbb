...
TEST(CUDAWaitFor, KernelCompletionSyncMode) {
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
    // Create a CUDA stream and event
    auto stream = cudaStream_t{};
    ASSERT_EQ(cudaStreamCreate(&stream), cudaSuccess);
    ...
    ASSERT_EQ(cudaEventRecord(event, stream), cudaSuccess);
    // Wait for the event to complete
    ASSERT_EQ(coopsync_tbb::cuda::wait_for(stream, 0), cudaSuccess);
    ASSERT_EQ(cudaEventQuery(event), cudaSuccess);
    ASSERT_EQ(cudaStreamSynchronize(stream),
              cudaSuccess);  // Ensure all the errors are propagated
    ASSERT_EQ(cudaGetLastError(), cudaSuccess);
...
TEST(CUDAWaitFor, MemoryOperationsSyncMode) {
    const std::size_t size = 100;
    ...
    ASSERT_EQ(cudaMemcpyAsync(host, device, size * sizeof(int),
              ...
              cudaSuccess);
    ASSERT_EQ(cudaEventRecord(event, stream), cudaSuccess);
    ASSERT_EQ(coopsync_tbb::cuda::wait_for(stream, 0u), cudaSuccess);
    ASSERT_EQ(cudaEventQuery(event), cudaSuccess);
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
    ASSERT_EQ(cudaGetLastError(), cudaSuccess);
    for (std::size_t i = 0; i < size; ++i) {
...
TEST(CUDAWaitFor, WaitForAllZeroStreamsSyncMode) {
    const auto errs = coopsync_tbb::cuda::wait_for_all(0u);
    ASSERT_EQ(errs.size(), 0u);
}

TEST(CUDAWaitFor, WaitForAllOneStreamSyncMode) {
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
    ...
    const auto errs = coopsync_tbb::cuda::wait_for_all(0u, stream);
    ASSERT_EQ(errs.size(), 1u);
    ASSERT_EQ(errs[0], cudaSuccess);
    ASSERT_EQ(cudaEventQuery(event), cudaSuccess);
    ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
...
TEST(CUDAWaitFor, WaitForAllTwoStreamsSyncMode) {
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
    ...
    const auto errs = coopsync_tbb::cuda::wait_for_all(0u, s0, s1);
    ASSERT_EQ(errs.size(), 2u);
    ASSERT_EQ(errs[0], cudaSuccess);
    ASSERT_EQ(errs[1], cudaSuccess);
    ASSERT_EQ(cudaEventQuery(e0), cudaSuccess);
...
TEST(CUDAWaitFor, WaitForRangeZeroStreamsSyncMode) {
    std::vector<cudaStream_t> streams;
    std::vector<cudaError_t> errs;

    const auto out = coopsync_tbb::cuda::wait_for_range(
        streams.begin(), streams.end(), std::back_inserter(errs), 0u);
    (void)out;
    ASSERT_TRUE(errs.empty());
}

TEST(CUDAWaitFor, WaitForRangeTwoStreamsSyncMode) {
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
