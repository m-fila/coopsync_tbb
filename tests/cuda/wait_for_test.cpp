...
TEST(CUDAWaitFor, WaitForAllDuplicatedStreams) {
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
    ...
    const auto errs = coopsync_tbb::cuda::wait_for_all(0u, s0, s1, s0, s1);
    ASSERT_EQ(errs.size(), 4u);
    ASSERT_EQ(errs[0], cudaSuccess);
    ASSERT_EQ(errs[1], cudaSuccess);
    ASSERT_EQ(errs[2], cudaSuccess);
    ASSERT_EQ(errs[3], cudaSuccess);
    ASSERT_EQ(cudaEventQuery(e0), cudaSuccess);
    ASSERT_EQ(cudaEventQuery(e1), cudaSuccess);
...
TEST(CUDAWaitFor, WaitForRangeDuplicatedStreams) {
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
